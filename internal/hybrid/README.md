...
Пакет реализует режим `hybrid`: для небольшого single-file/`stdin` input он может сразу положить весь текст в prompt, а для длинных файлов и директорий сначала собирает seeded tool history через `internal/aitools/seed`, после чего передаёт управление общему tool-calling loop.
...
- orchestration fused `search_rag` seed и synthetic verification reads до первого LLM turn через `internal/aitools/seed`;
...
4. До первого model turn `internal/aitools/seed` выполняет несколько synthetic `search_rag` вызовов по deterministic query variants.
...
- `hybrid` не декодирует concrete tool payload JSON сам: seeded history и evidence metadata приходят через shared tool-layer.
