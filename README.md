...
- В терминале финальный ответ рендерится как Markdown; при `--raw` или выводе в pipe/redirect печатается исходный текст в `stdout`, а runtime-логи при `--debug` идут в `stderr`.
...
- `--debug` включает подробные runtime-логи в `stderr`, не смешивая их с финальным ответом в `stdout`.
...
| `--debug`, `-d` | `DEBUG` | Подробные runtime-логи в `stderr` |
