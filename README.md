...
- `tools`: позволяет модели точечно искать нужные места в файле через инструменты (`search_file`, `read_lines`).
...
В режиме `tools` используется tool calling поверх файла (`search_file`/`read_lines`), а в `map-reduce` — чанкование и агрегация ответов.
...
│   │   ├── rag.go            # tools (Agentic Tool Calling) режим
...
| `--mode` | `MODE` | `map-reduce` | Режим: `map-reduce` или `tools` |
...
Цикл многошаговый: модель может несколько раз вызвать `search_file` и `read_lines`, пока не соберёт достаточно контекста для финального ответа.
...
Оба инструмента возвращают JSON, чтобы модель могла опираться на структурированные поля, а не на свободный текст.
...
- `search_file(query)` — поиск по файлу; запрос сначала трактуется как regex, а при невалидном шаблоне используется case-insensitive substring search. Возвращает JSON с `query`, `mode`, `match_count`, `matches`
